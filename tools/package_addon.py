...
Output: <out_dir>/upvn_editor_addon_v0.6.11.zip
...
Zip layout (v0.6.11, single top-level folder):
    upvn_editor_addon/
...
def build_addon_zip(out_path: pathlib.Path, with_template: bool = True):
    ...
    # M26g: a missing out_dir must be CREATED, not treated as a zip filename —
    # `python tools/package_addon.py` with no dist/ on disk used to write the
    # whole archive into a regular FILE literally named `dist`.
